...
class MihoyoPlugin(Star):
    ...
    def __init__(self, context: Context, config: AstrBotConfig = None):
        ...
        self._scheduler.add_job(
            self._daily_token_refresh,
            CronTrigger(hour=6, minute=0, timezone="Asia/Shanghai"),
            id="mihoyo_token_refresh",
            replace_existing=True,
        )
        self._scheduler.start()
        logger.info("[mihoyo] 每日 Token 自动刷新任务已启动（每天 06:00）")
    ...
    async def _daily_token_refresh(self):
        from .db.users import get_all_bound_qq_ids, force_refresh_tokens
        qq_ids = get_all_bound_qq_ids()
        logger.info(f"[mihoyo][refresh] 开始每日 Token 刷新，共 {len(qq_ids)} 个用户")
        ok = 0
        for qq_id in qq_ids:
            try:
                if await force_refresh_tokens(qq_id, proxy_url=self._login_proxy):
                    ok += 1
            except Exception as e:
                logger.warning(f"[mihoyo][refresh] qq={qq_id} 刷新失败: {e}")
        logger.info(f"[mihoyo][refresh] 刷新完成 {ok}/{len(qq_ids)} 成功")
