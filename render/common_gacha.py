...
_TEXTURE_DIR = Path(__file__).parent / "gacha" / "texture2D"
...
async def render_gacha_card(stats: dict, game: str, pool_type: str, uid: str, nickname: str = "") -> bytes:
    is_sr = game == "starrail"
    ...
    bg_path = first_existing(_VENDOR_GACHA_DIR / ("Abg3.png" if is_sr else "Abg1.png"), _TEXTURE_DIR / "bg.png")
    if bg_path:
        try:
    ...
    if nickname:
        draw.text((W - 32, 56), nickname, font=get_font(18), fill=GRAY, anchor="ra")
    ...
    for i, (label, val, col) in enumerate(cells):
        x0 = 32 + i * cell_w
        draw_rounded_rect(draw, (x0, y_g, x0 + cell_w - 8, y_g + 70), radius=8, fill=(40, 40, 60))
        draw.text((x0 + (cell_w - 8) // 2, y_g + 12), label, font=f_small, fill=GRAY, anchor="mt")
        draw.text((x0 + (cell_w - 8) // 2, y_g + 36), val, font=f_med, fill=col, anchor="mt")

    y_list = y_g + 90
    draw.text((32, y_list), "五星记录", font=f_small, fill=GRAY)
    y_list += 26

    five_stars = stats.get("five_star_list", [])
    if five_stars:
        for rec in five_stars[-8:]:
            name = rec.get("name", "?")
